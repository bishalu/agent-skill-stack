...
Generated by `scripts/render-docs.py` on 2026-09-03. Do not edit by hand — change `curation.json` and rebuild.
...
## `grilling`  ·  mattpocock  ·  CONDITIONAL

**Why.** Upstream trigger ('Use when the user wants to stress-test their thinking') is a near-owner-level collision with ce-brainstorm, which owns requirements and product framing. Narrowed to an explicit grill request plus delegation from grill-me, with an explicit non-ownership clause, following the tdd pattern. Left model-invocable on purpose: grill-me's body calls this skill by name, and disable-model-invocation would withhold it from the skill listing and break that delegation.
...
> Interview the user round by round to stress-test a plan, design, or decision they have already formed, mapping it as a design tree and settling one frontier of questions per round. Use when the user explicitly asks to be grilled, interrogated, or challenged on their reasoning, or when the grill-me entry point delegates here. A questioning technique applied to an existing idea; it does not own brainstorming, scoping what to build, or product framing, and it writes no plan artifact.
