...
- Two milestones may run at the same time when neither the spec nor their plans put them in sequence: neither consumes the other's output, they do not both write the same mutable dataset, and they do not rewrite the same modules. Give each its own `LANE_N`; milestones that share a dataset or modules share a lane. agent-sandbox holds one milestone lock per repository and lane, and the driver refuses a launch while `STATUS.md` shows another milestone in the same lane not yet pushed. Admission's memory budget bounds how many run; raise it for a parallel pair only when the host's memory log shows room, and remember a host-route gate in `integrate` is not counted by admission. Each milestone is still reviewed and integrated on its own, and the second `integrate` gates a tree that already holds the first.
  ...
  Decide "the same modules" from the files each milestone will actually edit, not from the screens it owns. Two milestones whose screens do not overlap still collide when both add an endpoint to one service module, a field to one schema, a recipe to one task file, or a dependency to one lock file; the shared files are usually the service's app and models, the task runner, the lock file and the generated API client. Read the spec sections for the endpoints and the data each milestone adds, and put them in one lane when those land in the same file. The second `integrate` then hits a conflict the driver refuses and cannot regenerate, and the cost is a follow-up turn to resolve it, which is more than the parallel run saved. `REGENERATE_ON_CONFLICT` covers only generated artifacts; a conflict in hand-written code is always a merge someone has to do.
...
An exhausted budget is a stop, not a retry. Write into `chain.log` what the attempts were and why each failed, then put the milestone in front of the owner with a recommendation: one more attempt (`approve N budget <name>`), a spec change, or a cut. Under `APPROVAL_MODE=supervisor` the supervisor grants that one more attempt itself and logs it with `granted_by=supervisor`, up to `MAX_SELF_EXTENSIONS` (default 2) per budget per milestone. Past that the milestone stops and waits for the owner whatever the mode, because a supervisor that always granted itself one more attempt would make the budget a slower loop rather than a stop, and exhaustion would never mean anything. A budget that keeps needing extensions is the signal to read: on milestone 11 it counted five failed gates that were all environment defects, not defects in the milestone's code. A budget key edited in config changes the gate definition, so raising a limit also needs `approve N gate-definition`.
