...
6. The decisions list against the spec section, reading for scope narrowed without saying so.
7. Test integrity. `integrate` refuses every weakening-scan hit (an added skip or xfail marker, a removed assertion, a deleted or modified existing test file, a changed snapshot, a gate-config change) that has no owner approval of its current blob. The report's "Test expectation changes" section is information for the owner and approves nothing. Read the listed diffs yourself, because the scan forces a change to be approved and cannot judge whether a rewritten assertion is weaker, and put each hit in front of the owner with your reading of it.
...
- starts with a `# criterion: <id or text>` line before its first diff, naming the exit criterion it breaks;
...
It gates a temporary merge of the sandbox branch into the integration branch head (the baseline, which must pass), applies the patch, gates again running every step, and appends the verdict to `.milestones/mutations/N.jsonl`. `caught` (an integration or replay step failed) is acceptance evidence for the cited criterion. `missed`, or `caught-static` (only lint or types noticed), becomes a finishing turn: the agent adds the test that fails on that defect. `inconclusive` means the step that would catch it is `sandbox-only` and did not run on the host; the mutation has to run where that step runs, which is `INTEGRATE_GATE_WHERE=sandbox`.
...
4. merges (with `--evidence`, the merge must already exist);
...
- `approve N weakening <hit kind> <path>...`: a weakening-scan hit (`skip-marker`, `removed-assert`, `deleted-test`, `test-change`, `snapshot`), approved for the path's current blob on the candidate branch, so a later change to the file is unapproved again.
